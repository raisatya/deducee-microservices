...
import { updateStudent } from "@/actions/administerServerActions"

import { PencilSquareIcon, XMarkIcon } from '@heroicons/react/24/outline'

export default function UpdateStudentDataModal({ currentUser, studentDetails }) {
    ...
    const handleSubmit = (event) => {
        event.preventDefault();
        startTransition(() => updateStudent(inputs, studentDetails._id));
        setIsOpen(false);
    }
                            ...
                            >
                                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    <form onSubmit={handleSubmit} className="h-full space-y-3 text-sm font-medium">
                                        ...
                                        <div className="flex justify-start items-center gap-3 divide-x my-2">
                                        <div className="">Student's name: {studentDetails.fullName}</div>
                                        <div className="px-3">Roll number: {studentDetails.rollNo}</div>
                                        </div>
