...
import { addClassroomNotices } from "@/actions/noticesServerActions";
import { InformationCircleIcon, ChevronRightIcon, XMarkIcon } from "@heroicons/react/20/solid"

const ClassroomNoticePublishForm = ({ currentUser, classroomId, classroomName }) => {
    ...
    const handleSubmit = (event) => {
        event.preventDefault();
        startTransition(() => addClassroomNotices(inputs, classroomId, classroomName, currentUser));
    }

    return <form onSubmit={handleSubmit} className="flex flex-col justify-end items-start space-y-4 bg-white px-6 pt-3 border rounded-lg">
        <p className="inline-flex items-center font-medium text-sm">Classroom Notices <span><ChevronRightIcon className="h-5 w-5 mr-1" /></span>{classroomName} <span><ChevronRightIcon className="h-5 w-5 mr-1" /></span>Publish</p>
...
export default ClassroomNoticePublishForm